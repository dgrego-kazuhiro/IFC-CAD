...
export interface DoorMeshParams {
    // Wall-frame coordinates: center of opening on wall axis, base on floor
    center: Vec3;
    axisDir: Vec3;   // unit direction along wall axis
    normalDir: Vec3; // unit normal (perpendicular in plan)
    width: number;
    height: number;
    thickness: number; // door panel thickness
}

// Simple door: a thin solid panel inside the opening, slightly inset from
// the wall's front face. 3D representation per spec §4.3 (簡易形状).
export class DoorMeshBuilder {
    public static build(p: DoorMeshParams): MeshData {
        ...
        // that would double up with the surrounding wall hole edges.
